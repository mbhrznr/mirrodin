"use client";

import type { FC } from "react";
import SyntaxHighlighter from "react-syntax-highlighter";
import type { SyntaxHighlighterProps } from "react-syntax-highlighter";

import {
  a11yDark,
  a11yLight,
} from "react-syntax-highlighter/dist/cjs/styles/hljs";

import useClipboard from "~/hooks/cilpboard/use-clipboard";
import useMediaQuery from "~/hooks/media-query/use-media-query";

import styles from "./code.block.styles.module.css";

/**
 * code block
 */
export default (function CodeBlock({ children, className, ...props }) {
  const { copy } = useClipboard();
  const dark = useMediaQuery("(prefers-color-scheme: dark)");
  const style = dark ? a11yDark : a11yLight;
  const [, language] = /language-(\w+)/.exec(className ?? "") ?? [];

  return (
    <pre className={styles.pre}>
      <SyntaxHighlighter
        {...(props as SyntaxHighlighterProps)}
        language={language}
        style={{
          ...style,
          hljs: { ...style.hljs, border: "1px solid var(--theme-border)" },
        }}
      >
        {String(children).replace(/\n$/, "")}
      </SyntaxHighlighter>
      <svg
        className={styles.svg}
        viewBox="0 0 24 24"
        width="28"
        onClick={() => copy(String(children))}
      >
        <path d="M16 1H4c-1.1 0-2 .9-2 2v14h2V3h12V1zm-1 4H8c-1.1 0-1.99.9-1.99 2L6 21c0 1.1.89 2 1.99 2H19c1.1 0 2-.9 2-2V11l-6-6zM8 21V7h6v5h5v9H8z"></path>
      </svg>
    </pre>
  );
} satisfies FC<
  JSX.IntrinsicElements["code"] & {
    inline?: boolean;
  }
>);
